...
export const executeTask = async (task: TaskExecution): Promise<TaskResult> => {
  logger.info(`Executing task ${task.executionId}...`);
  ...
  const runTask = (task: TaskExecution) => {
    ...
    executeTask(task)
      ...
      .catch((error: any) => {
        ...
        logger.error("Unexpected task execution failure:", error);
        sendMessage(
          "task:result",
    ...
    try {
      ...
      switch (type) {
        ...
        case "system:shutdown": {
          logger.info("Received shutdown command from server. Exiting...");
          process.exit(0);
        }
        default: {
